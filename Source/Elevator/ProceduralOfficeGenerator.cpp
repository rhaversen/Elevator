...
void AProceduralOfficeGenerator::PlaceElevator(const FOfficeElementDefinition& Element)
{
    ...
    const FVector2D Delta = Element.End - Element.Start;
    float TotalSpan = Delta.Size();
    FVector2D UnitDirection2D = FVector2D::ZeroVector;
    ...
    if (TotalSpan > KINDA_SMALL_NUMBER)
    {
        UnitDirection2D = Delta / TotalSpan;
        Center2D = (Element.Start + Element.End) * 0.5f;
        YawDegrees = FMath::Atan2(UnitDirection2D.Y, UnitDirection2D.X) * (180.0f / PI);
    }
    else
    {
        ...
        if (DefaultSpan <= KINDA_SMALL_NUMBER && ElevatorMeshPtr)
        {
    ...
    if (bSpawnElevatorLightComponents && ElevatorLightIntensity > KINDA_SMALL_NUMBER)
    {
        ...
        if (NewLight)
        {
            ...
            // Get elevator mesh height if available
