#include "UI/Programs/TrailMakingTestProgram.h"
...
REGISTER_SCREEN_PROGRAM(FTrailMakingTestProgram, "trail_making_test")
...
int32 STrailWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
    ...
    if (Program->HasFailureFade())
    {
        if (FailureAlpha > 0.0f)
        {
            ...
            for (const FTrailMakingTestProgram::FTrailSegment& Segment : Program->FailureSegments)
            {
...
// ============================================================================
// FTrailMakingTestProgram Implementation
// ============================================================================

FTrailMakingTestProgram::FTrailMakingTestProgram()
    : FTrialProgramBase(5)
{
    SetTrialTitle(FText::FromString(TEXT("Trail Making Test")));
    GenerateNodes();
}

void FTrailMakingTestProgram::GenerateNewTrial()
{
    CurrentPhase = ERoutePhase::PhaseA;
    GenerateNodes();
}

void FTrailMakingTestProgram::GenerateNodes()
{
    Nodes.Empty();
    CurrentTarget = 0;
    bDragging = false;
...
void FTrailMakingTestProgram::ResetTrial()
{
    for (auto& Node : Nodes)
    {
        Node.bVisited = false;
...
void FTrailMakingTestProgram::CheckNodeIntersection(const FVector2D& NormalizedPos)
{
...
void FTrailMakingTestProgram::InvalidateTrail() const
{
...
void FTrailMakingTestProgram::ScheduleAdvance(EAdvanceAction Action)
{
...
void FTrailMakingTestProgram::ProcessPendingAdvance()
{
    if (!bPendingAdvance)
    {
        return;
...
void FTrailMakingTestProgram::StartFailureFade()
{
    FailureSegments.Reset();
...
void FTrailMakingTestProgram::ClearFailureFade()
{
...
float FTrailMakingTestProgram::GetFailureFadeAlpha(double Now) const
{
...
FText FTrailMakingTestProgram::GetStatusText() const
{
    if (GetTaskComplete())
    {
...
TSharedRef<SWidget> FTrailMakingTestProgram::BuildTrialBody()
{
...
void FTrailMakingTestProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FTrailMakingTestProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FTrailMakingTestProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
FVector2D FTrailMakingTestProgram::GetNormalizedPosition(const FVector2D& PixelPosition, FVector2D* OutLocalPosition) const
{
