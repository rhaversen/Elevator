#include "UI/Programs/TowerOfHanoiProgram.h"
...
REGISTER_SCREEN_PROGRAM(FTowerOfHanoiProgram, "tower_of_hanoi")

FTowerOfHanoiProgram::FTowerOfHanoiProgram()
    : FTrialProgramBase(1)
{
    SetTrialTitle(FText::FromString(TEXT("Tower of Hanoi")));
    SetTaskComplete(false);
    InitializeBlocks();
}

void FTowerOfHanoiProgram::GenerateNewTrial()
{
    InitializeBlocks();
    SetTaskComplete(false);
}

void FTowerOfHanoiProgram::InitializeBlocks()
{
    Pegs.SetNum(3);
...
bool FTowerOfHanoiProgram::CanMoveBlock(int32 FromPeg, int32 ToPeg) const
{
...
void FTowerOfHanoiProgram::MoveBlock(int32 FromPeg, int32 ToPeg)
{
...
bool FTowerOfHanoiProgram::CheckWinCondition() const { return Pegs[2].Num() == NumBlocks; }

int32 FTowerOfHanoiProgram::GetPegAtPosition(const FVector2D& Position) const
{
...
FVector2D FTowerOfHanoiProgram::GetPegPosition(int32 PegIndex) const
{
    const FVector2D Size = GetProgramSize();
...
FString FTowerOfHanoiProgram::RenderPegColumn(int32 PegIndex) const
{
...
TSharedRef<SWidget> FTowerOfHanoiProgram::BuildTrialBody()
{
...
void FTowerOfHanoiProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FTowerOfHanoiProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    if (bHandledByPreProcessor) return;
...
void FTowerOfHanoiProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    ActivePointerKey = EKeys::Invalid;
}
