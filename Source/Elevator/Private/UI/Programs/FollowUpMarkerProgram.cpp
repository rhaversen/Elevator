#include "UI/Programs/FollowUpMarkerProgram.h"
...
float FFollowUpMarkerProgram::GetAnimProgress() const
{
...
void FFollowUpMarkerProgram::InvalidateWidget() const
{
...
float FFollowUpMarkerProgram::GetTransitionAlpha() const
{
...
FVector2D FFollowUpMarkerProgram::GetSlotSize() const
{
    return FVector2D(200.0f, 130.0f);
}

FVector2D FFollowUpMarkerProgram::GetMatchButtonCenter() const
{
...
FVector2D FFollowUpMarkerProgram::GetMatchButtonSize() const
{
    return FVector2D(80.0f, 80.0f);  // Square for equals sign
}

bool FFollowUpMarkerProgram::IsPointInRect(const FVector2D& Point, const FVector2D& Center, const FVector2D& Size) const
{
...
void FFollowUpMarkerProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    if (bHandledByPreProcessor || GetTaskComplete()) return;
...
void FFollowUpMarkerProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    bMatchPressed = false;
