#include "UI/Programs/CorsiBlockTappingProgram.h"
...
REGISTER_SCREEN_PROGRAM(FCorsiBlockTappingProgram, "corsi_block_tapping")

FCorsiBlockTappingProgram::FCorsiBlockTappingProgram()
    : FTrialProgramBase(MaxLength)
{
    SetTrialTitle(FText::FromString(TEXT("Corsi Block-Tapping Task")));
    SetTaskComplete(false);
    GenerateSequence();
}

void FCorsiBlockTappingProgram::GenerateNewTrial()
{
    SetTaskComplete(false);
    GenerateSequence();
}

void FCorsiBlockTappingProgram::GenerateSequence()
{
...
void FCorsiBlockTappingProgram::UpdateState()
{
    const double Now = FPlatformTime::Seconds();
...
void FCorsiBlockTappingProgram::StartInputPhase()
{
...
void FCorsiBlockTappingProgram::StartFeedback(bool bSuccess, int32 ErrorBlock, TOptional<FText> CustomMessage)
{
...
const TArray<FCorsiBlockTappingProgram::FTabDescriptor>& FCorsiBlockTappingProgram::GetTabDescriptors()
{
...
const FCorsiBlockTappingProgram::FTabDescriptor& FCorsiBlockTappingProgram::GetTabDescriptor(int32 BlockIndex) const
{
...
int32 FCorsiBlockTappingProgram::GetBlockAtPosition(const FVector2D& Position) const
{
...
void FCorsiBlockTappingProgram::TapBlock(int32 BlockIndex)
{
...
FText FCorsiBlockTappingProgram::GetStatusText() const
{
    return StatusMessage;
}

float FCorsiBlockTappingProgram::GetResponseProgress() const
{
...
FSlateColor FCorsiBlockTappingProgram::GetBlockTextColor(int32 BlockIndex) const
{
...
TSharedRef<SWidget> FCorsiBlockTappingProgram::BuildTrialBody()
{
    const FScreenProgramStyle& Style = GetProgramStyle();
...
void FCorsiBlockTappingProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FCorsiBlockTappingProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    if (bHandledByPreProcessor || GetTaskComplete())
    {
        return;
...
void FCorsiBlockTappingProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
