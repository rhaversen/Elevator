#include "UI/Programs/FlickerChangeDetectionProgram.h"
...
REGISTER_SCREEN_PROGRAM(FFlickerChangeDetectionProgram, "flicker_change_detection")
    ...
    FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
    ...
    const float CellWidth = Size.X / FFlickerChangeDetectionProgram::GridCols;
    const float CellHeight = Size.Y / FFlickerChangeDetectionProgram::GridRows;
    
    // Check if we're in a blank state
    const bool bIsBlank = (Program->ViewState == FFlickerChangeDetectionProgram::EViewState::BlankBeforeB ||
                           Program->ViewState == FFlickerChangeDetectionProgram::EViewState::BlankBeforeA);
    const bool bShowingB = (Program->ViewState == FFlickerChangeDetectionProgram::EViewState::ShowingB);
    
    // Draw grid lines (always visible, even during blank)
    for (int32 Col = 0; Col <= FFlickerChangeDetectionProgram::GridCols; ++Col)
    {
    ...
    for (int32 Row = 0; Row <= FFlickerChangeDetectionProgram::GridRows; ++Row)
    {
        TArray<FVector2D> Line;
    ...
    for (int32 Row = 0; Row < FFlickerChangeDetectionProgram::GridRows; ++Row)
    {
        for (int32 Col = 0; Col < FFlickerChangeDetectionProgram::GridCols; ++Col)
        {
            const int32 CellIdx = Row * FFlickerChangeDetectionProgram::GridCols + Col;
...
// ============================================================================
// FFlickerChangeDetectionProgram Implementation
// ============================================================================

FFlickerChangeDetectionProgram::FFlickerChangeDetectionProgram()
    : FTrialProgramBase(6)
{
    SetTrialTitle(FText::FromString(TEXT("Flicker Change Detection Task")));
    LastStateChangeTime = FPlatformTime::Seconds();
    GenerateDifference();
}

void FFlickerChangeDetectionProgram::GenerateNewTrial()
{
    GenerateDifference();
}

void FFlickerChangeDetectionProgram::UpdateFlicker()
{
...
void FFlickerChangeDetectionProgram::GenerateDifference()
{
...
double FFlickerChangeDetectionProgram::GetBlankDuration(bool bBeforeB) const
{
...
int32 FFlickerChangeDetectionProgram::GetCellAtPosition(const FVector2D& NormalizedPos) const
{
...
void FFlickerChangeDetectionProgram::InvalidateWidget() const
{
...
TSharedRef<SWidget> FFlickerChangeDetectionProgram::BuildTrialBody()
{
...
void FFlickerChangeDetectionProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FFlickerChangeDetectionProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    if (bHandledByPreProcessor || GetTaskComplete()) return;
...
void FFlickerChangeDetectionProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
