#include "UI/Programs/TimeDriftSynchronizerProgram.h"
...
// ============================================================================
// FTimeDriftSynchronizerProgram Implementation
// ============================================================================
...
void FTimeDriftSynchronizerProgram::InitializeClock()
{
    Clock = FClockState();
    GenerateTargetTime();
...
FVector2D FTimeDriftSynchronizerProgram::GetClockCenter() const
{
...
float FTimeDriftSynchronizerProgram::GetClockRadius() const
{
    // Match the SClockWidget radius calculation
    return 300.0f * 0.45f; // 300 is widget size, 0.45 is radius factor
}

FVector2D FTimeDriftSynchronizerProgram::GetHourHandlePosition() const
{
...
FVector2D FTimeDriftSynchronizerProgram::GetMinuteHandlePosition() const
{
    FVector2D Center = GetClockCenter();
    float Radius = GetClockRadius();
...
float FTimeDriftSynchronizerProgram::AngleFromPosition(const FVector2D& Pos) const
{
    FVector2D Center = GetClockCenter();
...
void FTimeDriftSynchronizerProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FTimeDriftSynchronizerProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    if (bHandledByPreProcessor) return;
...
void FTimeDriftSynchronizerProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    ActivePointerKey = EKeys::Invalid;
    DragTarget = EDragTarget::None;
}

bool FTimeDriftSynchronizerProgram::CheckTimeMatch() const
{
    float targetHourAngle = GetAngleForHour(Clock.TargetHour, Clock.TargetMinute);
    float targetMinuteAngle = GetAngleForMinute(Clock.TargetMinute);
...
float FTimeDriftSynchronizerProgram::GetAngleForHour(int32 Hour, int32 Minute) const
{
    return (Hour % 12) * 30.0f + Minute * 0.5f;
}

float FTimeDriftSynchronizerProgram::GetAngleForMinute(int32 Minute) const
{
    return Minute * 6.0f;
}

float FTimeDriftSynchronizerProgram::NormalizeAngle(float Angle) const
{
