#include "UI/Programs/MentalRotationProgram.h"
...
REGISTER_SCREEN_PROGRAM(FMentalRotationProgram, "mental_rotation")
...
// ============================================================================
// FMentalRotationProgram Implementation
// ============================================================================

FMentalRotationProgram::FMentalRotationProgram()
    : FTrialProgramBase(8)
{
    SetTrialTitle(FText::FromString(TEXT("Mental Rotation Task")));
    GenerateNewTrial();
}

void FMentalRotationProgram::GenerateNewTrial()
{
...
bool FMentalRotationProgram::CheckMatch() const
{
...
void FMentalRotationProgram::ApplyAlignment()
{
    if (CheckMatch())
    {
        AdvanceTrial();
...
FVector2D FMentalRotationProgram::GetAdjustCenter(const FVector2D& WidgetSize) const
{
...
float FMentalRotationProgram::GetMarkSize(const FVector2D& WidgetSize) const
{
    const float ControlAreaHeight = 70.0f;
    const float PaneWidth = WidgetSize.X * 0.5f;
    const float PaneHeight = WidgetSize.Y - ControlAreaHeight;
...
float FMentalRotationProgram::GetRingRadius(const FVector2D& WidgetSize) const
{
    return GetMarkSize(WidgetSize) * 0.9f;
}

float FMentalRotationProgram::AngleFromPosition(const FVector2D& Pos, const FVector2D& Center) const
{
...
float FMentalRotationProgram::NormalizeAngle(float Angle) const
{
...
EMarkAlignmentDragTarget FMentalRotationProgram::GetHitTarget(const FVector2D& LocalPos, const FVector2D& WidgetSize) const
{
    const float ControlAreaHeight = 70.0f;
    const float PaneHeight = WidgetSize.Y - ControlAreaHeight;
    const float MirrorControlY = PaneHeight + 8.0f;
...
void FMentalRotationProgram::InvalidateWidget() const
{
...
TSharedRef<SWidget> FMentalRotationProgram::BuildTrialBody()
{
...
void FMentalRotationProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FMentalRotationProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    if (bHandledByPreProcessor || GetTaskComplete()) return;
...
void FMentalRotationProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
