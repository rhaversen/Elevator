#include "UI/Programs/DirectoryIndexRebuildProgram.h"
    ...
    const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
    ...
    const float CellSize = FDirectoryIndexRebuildProgram::CellSize;
    const int32 GridCols = FDirectoryIndexRebuildProgram::GridCols;
    const int32 GridRows = FDirectoryIndexRebuildProgram::GridRows;
    ...
    // Grid area
    const FDirectoryIndexRebuildProgram::FCanvasLayout Layout = Program->BuildCanvasLayout(Size);
    ...
    // Apply button (always visible)
    {
        ...
        const float BtnWidth = FDirectoryIndexRebuildProgram::ApplyButtonWidth;
        const float BtnHeight = FDirectoryIndexRebuildProgram::ApplyButtonHeight;
...
FDirectoryIndexRebuildProgram::FCanvasLayout FDirectoryIndexRebuildProgram::BuildCanvasLayout(const FVector2D& CanvasSize) const
{
...
int32 FDirectoryIndexRebuildProgram::FindGridCellAtPosition(const FVector2D& LocalPosition, const FCanvasLayout& Layout) const
{
...
int32 FDirectoryIndexRebuildProgram::FindPaletteItemAtPosition(const FVector2D& LocalPosition, const FCanvasLayout& Layout) const
{
...
bool FDirectoryIndexRebuildProgram::IsOverApplyButton(const FVector2D& LocalPosition, const FCanvasLayout& Layout) const
{
...
bool FDirectoryIndexRebuildProgram::IsApplyButtonClickable() const
{
    return !bShowingTarget && !AreAllTrialsComplete();
}

void FDirectoryIndexRebuildProgram::GenerateNewTrial()
{
    TargetPattern.Empty();
    for (int32& V : UserGrid) V = -1;
...
void FDirectoryIndexRebuildProgram::CheckAndApply()
{
...
void FDirectoryIndexRebuildProgram::InvalidateWidget() const
{
...
float FDirectoryIndexRebuildProgram::GetPreviewFadeAlpha(double Now) const
{
    if (bShowingTarget)
    {
        return 1.0f;
...
bool FDirectoryIndexRebuildProgram::IsPreviewFadeActive(double Now) const
{
    return PreviewFadeStartTime > 0.0 && (Now - PreviewFadeStartTime) < PreviewFadeDurationSeconds;
}

float FDirectoryIndexRebuildProgram::GetErrorFlashAlpha(double Now) const
{
...
TSharedRef<SWidget> FDirectoryIndexRebuildProgram::BuildTrialBody()
{
...
void FDirectoryIndexRebuildProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FDirectoryIndexRebuildProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
    if (bHandledByPreProcessor || AreAllTrialsComplete()) return;
    const TSharedPtr<SDirectoryIndexRebuildWidget> CanvasPtr = CanvasWidget.Pin();
    if (!CanvasPtr.IsValid()) return;
...
void FDirectoryIndexRebuildProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
