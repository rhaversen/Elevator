#include "UI/Programs/IowaGamblingTaskProgram.h"
...
REGISTER_SCREEN_PROGRAM(FIowaGamblingTaskProgram, "iowa_gambling_task")
...
// ============================================================================
// FIowaGamblingTaskProgram Implementation
// ============================================================================

FIowaGamblingTaskProgram::FIowaGamblingTaskProgram()
    : FTrialProgramBase(3)
{
    SetTrialTitle(FText::FromString(TEXT("Iowa Gambling Task")));
...
namespace
{
    void BuildDeckPattern(int32 DeckIndex, TArray<FIowaGamblingTaskProgram::FDeckOutcome>& OutDeck)
    {
        OutDeck.Reset();
        switch (DeckIndex)
        {
            ...
            {
                const FIowaGamblingTaskProgram::FDeckOutcome Pattern[] = {
        ...
        {
            const FIowaGamblingTaskProgram::FDeckOutcome Copy = OutDeck[Index];
            OutDeck.Add(Copy);
        }
        Algo::RandomShuffle(OutDeck);
    }
...
void FIowaGamblingTaskProgram::GenerateNewTrial()
{
    Score = 0;
    QuotaGoal = 100;
    DrawsRemaining = 16;
...
FIowaGamblingTaskProgram::FDeckOutcome FIowaGamblingTaskProgram::ConsumeOutcome(int32 DeckIndex)
{
...
void FIowaGamblingTaskProgram::SelectDeck(int32 DeckIndex)
{
...
FString FIowaGamblingTaskProgram::GetDeckLabel(int32 DeckIndex) const
{
...
FVector2D FIowaGamblingTaskProgram::GetDeckCardTopLeft(int32 DeckIndex, const FVector2D& WidgetSize) const
{
...
int32 FIowaGamblingTaskProgram::GetDeckAtPosition(const FVector2D& LocalPos, const FVector2D& WidgetSize) const
{
    for (int32 DeckIndex = 0; DeckIndex < 4; ++DeckIndex)
    {
...
void FIowaGamblingTaskProgram::InvalidateWidget() const
{
...
TSharedRef<SWidget> FIowaGamblingTaskProgram::BuildTrialBody()
{
...
void FIowaGamblingTaskProgram::HandlePointerMoved(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FIowaGamblingTaskProgram::HandlePointerPressed(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
...
void FIowaGamblingTaskProgram::HandlePointerReleased(const FScreenPointerEvent& Event, bool bHandledByPreProcessor)
{
}
